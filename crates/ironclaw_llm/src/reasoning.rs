...
/// Semantic fingerprint for deduplication (hash of normalized content lines).
fn compute_content_fingerprint(content: &str) -> u64 {
...
/// Check if content is semantically similar to existing messages (>70% line overlap).
fn is_semantic_duplicate(content: &str, existing_sections: &[(String, &str)]) -> bool {
    let content_lines: HashSet<&str> = content
        .lines()
...
/// Infer section label from content (heuristic based on keywords).
fn infer_section_label(content: &str) -> &'static str {
    let lower = content.to_lowercase();
    if lower.contains("tool") && lower.contains("call") {
        "Tool Guidance"
    } else if lower.contains("safety") || lower.contains("comply") {
        "Safety & Policy"
    } else if lower.contains("skill") || lower.contains("capability") {
        "Skills & Capabilities"
    } else if lower.contains("channel") || lower.contains("format") {
        "Channel Formatting"
    } else if lower.contains("think") || lower.contains("response format") {
        "Response Format"
    } else if lower.contains("extension") || lower.contains("search") {
        "Extensions"
    } else if lower.contains("group") || lower.contains("chat") {
        "Group Chat"
    } else {
        "Additional Guidance"
    }
