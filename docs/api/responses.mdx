...
| `model` | no | Optional per-request model override. Omit it (or set `"default"`) to use normal routing; provide a provider model id (for example `"qwen3.6:27b"`) to request that model for this turn. |
...
| `suppress_user_context` | no | IronClaw extension. When `true`, keeps `x_context` in request metadata but skips prepending it into the model prompt as `<user-context>`. Default `false`. |
...
`model` is validated as a non-empty string. Extremely long model identifiers are rejected.
...
If you need the context to remain available for metadata/audit purposes but do not want it injected into the model prompt, send `"suppress_user_context": true` alongside `x_context`.
